...
\begin{frame}{Quel temps de traversée ? Le temps de groupe de Hartman}
  ...
  \]
  Le paquet tunnel sort \emph{plus tôt} que le paquet libre : c'est
  l'\textbf{effet Hartman}.
\end{frame}
...
\begin{frame}{Influence du paquet : impulsion moyenne $k_0$}
  \begin{center}
    \includegraphics[width=0.82\textwidth]{Figure_5_influence_k0.png}
  \end{center}
  \begin{itemize}
    ...
    \item Pas de violation de causalité : la barrière ne transmet que
          l'avant-garde rapide du spectre — le « pic » transmis est reconstruit
          à partir de composantes déjà en avance.
  \end{itemize}
\end{frame}
...
\begin{frame}{Influence du paquet : largeur initiale $a_{\rm wp}$}
  \begin{center}
    \includegraphics[width=0.82\textwidth]{Figure_6_influence_awp.png}
  \end{center}
  \begin{itemize}
    ...
    \item Paquet étroit : la barrière transmet préférentiellement les
          composantes rapides $\Rightarrow$ sortie plus précoce et transmission
          jusqu'à $8$ fois supérieure à la prédiction onde plane.
  \end{itemize}
\end{frame}
