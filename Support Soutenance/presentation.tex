...
b_mat = (1 + 2*r) + s_int
      \end{tcblisting}
      \codesource{evolution\_schrodinger() --- Partie 4}
    \end{column}
  \end{columns}
  \vspace{0.1em}
...
\begin{frame}[fragile]{Validation : particule libre ($V_0 = 0$)}
  ...
  zone $[0, a]$ :
  \[
    \tikzmarknode{eqtau}{\color{bordeaux}$\tau_0^{\rm num} = t_{\rm sortie} - t_{\rm entrée}$}
...
(*\tikzmarknode{codetrans}{\color{codeaccent}\bfseries norme\_trans}*) = norme(
  ...
  \begin{tikzpicture}[remember picture, overlay]
    \draw[-{Triangle[length=2mm]}, thick, bordeaux]
      (codetrans.south) to[out=-90,in=90] (eqtrans.north);
  \end{tikzpicture}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
      \begin{block}{Temps de traversée théorique}
        \[
          \tikzmarknode{eqtaut}{\color{bordeaux}$\tau_t^{\rm th} = \tau_0 + \tau_g$}
        \]
...
return (*\tikzmarknode{codetaut}{\color{codeaccent}\bfseries tau0 + tau\_g}*)
  ...
  \begin{tikzpicture}[remember picture, overlay]
    \draw[-{Triangle[length=2mm]}, thick, bordeaux]
      (eqtaut.east) to[out=0,in=180] (codetaut.west);
  \end{tikzpicture}
\end{frame}
